...
def devolver_bicicletas_inicio(estaciones, bicicletas,usuarios,usuarios_bloqueados,viajes_actuales,viajes_finalizados):
    ...
    with open('Ecobici/TP2/viajes_en_curso.pkl','rb') as archivo: 
        seguir = True
        while seguir: 
            try:
...
def lectura_estaciones():
    lista_estaciones = recorrer_archivo('Ecobici/TP2/estaciones.csv')
    estaciones = {}
...
def lectura_bicicletas(estaciones):
    lista_bicicletas = recorrer_archivo('Ecobici/TP2/bicicletas.csv')
    bicicletas = {}
    #estado,ubicacion
    estado  = "ok"
    ubicacion = "anclada"
...
def lectura_usuarios(usuarios_bloqueados):
    lista_usuarios = recorrer_archivo_completo('Ecobici/TP2/maestro_usuarios.csv')
    usuarios = {}
...
def alta_usuario(usuarios):
    ...
    if not dni in usuarios:
        ...
        maestro_usuarios2 = open('Ecobici/TP2/maestro_usuarios.csv','a',encoding = 'utf-8')
...
def informe_cantidad_viajes(usuarios):
    lista_viajes = recorrer_archivo('Ecobici/TP2/viajes.csv')
...
def suma_horarios_viajes(pre, post):
    viaje_previo = pre.split(':')
    viaje_posterior = post.split(':')
...
def devolver_bicicleta(forma_de_uso, dni,estacion,estaciones,bicicletas,usuarios,usuarios_bloqueados,viajes_actuales,viajes_finalizados):
    ...
    if len(estaciones[estacion][4]) <= 29: #se fija que haya lugar en la estacion
        ...
        if duracion_viaje > 60:
            ...
            print("la distancia recorrida fue de: {} kms.".format(distancia_recorrida))
        linea_viaje_finalizado = "{},{},{},{},{},{},{} \n".format(estacion_origen, estacion, dni,hora_salida, duracion_viaje, horario_llegada, numero_bicicleta)
        archivo_viajes_finalizados = open('Ecobici/TP2/viajes.csv', 'a', encoding = 'utf-8')
    ...
    for dni_ladron, usuario in usuarios.items():
        if dni == dni_ladron:
            if(bloquear):
                ...
                archivo_viajes_robados = open('Ecobici/TP2/viajes_robados.csv', 'a', encoding = 'utf-8')
...
def informe_viajes_robados ():
    lista_viajes_robados = recorrer_archivo_completo('Ecobici/TP2/viajes_robados.csv')
